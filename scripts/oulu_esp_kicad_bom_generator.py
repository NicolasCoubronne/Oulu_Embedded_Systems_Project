...
writerow( out, ['Date:', datetime.date.isoformat(datetime.date.today())] )
...
writerow( out, [] )                        # blank line
writerow( out, ['Item', 'Qty', 'Reference(s)', 'Value', 'Price (individual, euros)', 'Price (total, euros)', 'Purchase link & datasheet'])
...
for group in grouped:
    ...
    price = float(net.getGroupField(group, columns[4]))
    # price field, individual
    row.append('{:.2f}'.format(price))
    # price field, sum  
    row.append('{:.2f}'.format(price * len(group)))
...
# write total price
writerow(out, [])
writerow(out, ['Total Price', '', '', '', '', '{:.2f}'.format(total_price), 'euros'])
